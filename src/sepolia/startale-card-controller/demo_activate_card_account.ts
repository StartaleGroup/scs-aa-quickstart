...
const main = async () => {
  ...
  try {
    ...
    // Deterministic salt — same formula used in demo_make_card_account.ts
    const salt = keccak256(toHex(userAccount)) as Hex;

    // Predict the clone address by simulating as the smart account — satisfies
    // the msg.sender == userAccount check without an on-chain tx.
    ...
    const label = !isDeployed && !isApproved
      ? "makeNewCardAccount + USDSC.approve"
      : !isDeployed ? "makeNewCardAccount" : "USDSC.approve";
